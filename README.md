...
| `--ignore-features <slug,...>` | Ignore specific unknown/partial features (defaults to `tag:html,tag:head,tag:body`)        |
...
- To keep reports focused, the CLI ignores `tag:html`, `tag:head`, and `tag:body` by default. Extend the ignore list via
  `--ignore-features`/`EHV_IGNORE_FEATURES` when you want to suppress additional features.
...
| `EHV_IGNORE_FEATURES` | Comma-separated features to hide (same as `--ignore-features`) |
